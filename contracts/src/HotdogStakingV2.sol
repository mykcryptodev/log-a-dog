...
/**
 * @title HotdogStakingV2
 * @dev Season 4 staking contract for HOTDOG tokens with proportional rewards.
 */
contract HotdogStakingV2 is AccessControl, ReentrancyGuard {
    ...
    // Season 4: July 4, 2026 10:00:00 ET through September 7, 2026 23:59:00 ET.
    uint256 public constant SEASON_START_TIME = 1783173600; // July 4, 2026 14:00:00 UTC
    uint256 public constant REWARD_END_TIME = 1788839940; // September 8, 2026 03:59:00 UTC
    ...
    constructor(address _hotdogToken) {
        ...
        lastRewardUpdate = block.timestamp < SEASON_START_TIME ? SEASON_START_TIME : block.timestamp;
    ...
    function updateGlobalRewards() internal {
        if (block.timestamp <= SEASON_START_TIME) {
            lastRewardUpdate = SEASON_START_TIME;
            return;
        }
        ...
        if (lastRewardUpdate >= REWARD_END_TIME) {
            return;
        }

        uint256 effectiveTimestamp = block.timestamp > REWARD_END_TIME ? REWARD_END_TIME : block.timestamp;
        if (effectiveTimestamp <= lastRewardUpdate) {
            return;
        }
        uint256 timeElapsed = effectiveTimestamp - lastRewardUpdate;
        if (timeElapsed == 0) return;

        uint256 rewardsToDistribute;
        if (effectiveTimestamp == REWARD_END_TIME) {
            rewardsToDistribute = rewardsPool;
        } else {
            uint256 timeRemaining = REWARD_END_TIME - effectiveTimestamp;
            // Calculate emission rate: rewards per second = rewardsPool / timeRemaining
            rewardsToDistribute = (rewardsPool * timeElapsed) / timeRemaining;
        }
        ...
        lastRewardUpdate = effectiveTimestamp;
    ...
    function getPendingRewards(address user) public view returns (uint256) {
        ...
        if (block.timestamp > SEASON_START_TIME && lastRewardUpdate < REWARD_END_TIME) {
            uint256 effectiveTimestamp = block.timestamp > REWARD_END_TIME ? REWARD_END_TIME : block.timestamp;
            uint256 timeElapsed = effectiveTimestamp - lastRewardUpdate;
    ...
    function getCurrentEmissionRate() external view returns (uint256) {
        if (block.timestamp < SEASON_START_TIME || block.timestamp >= REWARD_END_TIME || rewardsPool == 0) {
            return 0;
        }
    ...
    /**
     * @notice Get time remaining until the season opens
     * @return Time remaining in seconds
     */
    function getTimeUntilStart() external view returns (uint256) {
        ...
        return SEASON_START_TIME - block.timestamp;
    }

    /**
     * @notice Check whether new stakes are currently allowed
     * @return Whether the season is currently active
     */
    function isSeasonActive() external view returns (bool) {
        return block.timestamp >= SEASON_START_TIME && block.timestamp < REWARD_END_TIME;
    }
