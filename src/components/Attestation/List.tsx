...
} from "~/stores/pendingTransactions";
...
export const ListAttestations: FC<Props> = ({ limit }) => {
  ...
  const { getPendingDogsForChain, clearExpiredPending } =
    usePendingTransactionsStore();
  ...
  const filteredPendingDogs = pendingDogs.filter((pending) => {
    ...
    if (hasRealData) {
      console.log(
        `🔄 Filtering out optimistic dog ${pending.logId} - real data found`,
      );
    }
    return !hasRealData;
  });
  const allHotdogs: HotdogItem[] = useMemo(
    () =>
      dogData?.hotdogs
        ? [...filteredPendingDogs, ...dogData.hotdogs]
        : pendingDogs,
      ...
      <>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="join place-content-center md:col-span-2">
            ...
            <button className="btn join-item">Page 1 of ...</button>
            <button className="btn join-item" disabled>
              »
            </button>
          </div>
  ...
  const validMap =
    dogData?.hotdogs && dogData.validAttestations
      ? Object.fromEntries(
          dogData.hotdogs.map((h, i) => [
            h.logId,
            dogData.validAttestations[i],
          ]),
        )
      : {};
  const invalidMap =
    dogData?.hotdogs && dogData.invalidAttestations
      ? Object.fromEntries(
          dogData.hotdogs.map((h, i) => [
            h.logId,
            dogData.invalidAttestations[i],
          ]),
        )
      : {};
  const userAttestedMap =
    dogData?.hotdogs && dogData.userAttested
      ? Object.fromEntries(
          dogData.hotdogs.map((h, i) => [h.logId, dogData.userAttested[i]]),
        )
      : {};
  const userAttestationMap =
    dogData?.hotdogs && dogData.userAttestations
      ? Object.fromEntries(
          dogData.hotdogs.map((h, i) => [h.logId, dogData.userAttestations[i]]),
        )
      : {};

  // Helper function to get attestation data for a hotdog
  const getAttestationData = (
    hotdog: HotdogItem,
  ): {
    ...
    <>
      ...
      <div className="flex flex-col gap-4">
        {allHotdogs.map((hotdog) => {
          console.log("🌭 Rendering hotdog:", hotdog.logId, {
            isPending: "isPending" in hotdog && hotdog.isPending,
          });
          const isExpired =
            Number(hotdog.timestamp) * 1000 +
              ATTESTATION_WINDOW_SECONDS * 1000 <=
            Date.now();
          const attestationData = getAttestationData(hotdog);
          const isPending = "isPending" in hotdog && hotdog.isPending;
          return (
            <div
              className="card bg-base-200 bg-opacity-25 shadow backdrop-blur-sm"
              key={hotdog.logId}
            >
              <div className="card-body max-w-lg p-4">
                <div className="flex items-center justify-between">
                  ...
                  <Revoke hotdog={hotdog} onRevocation={refetchDogData} />
                </div>
                {"zoraCoin" in hotdog &&
                  hotdog.zoraCoin &&
                    ...
                    <div className="flex w-full items-center justify-between text-xs opacity-50">
                      <div className="flex items-center gap-0.5">
                        ...
                        {formatAbbreviatedFiat(
                          Number(hotdog.zoraCoin.marketCap),
                        )}
                      </div>
                      <div className="flex items-center gap-0.5">
                        <FireIcon className="h-4 w-4" /> 24H VOL $
                        {formatAbbreviatedFiat(
                          Number(hotdog.zoraCoin.volume24h),
                        )}
                      </div>
                    </div>
                  )}
                <Link href={`/dog/${hotdog.logId}`} className="w-fit">
                  <HotdogImage
                    src={hotdog.imageUri}
                ...
                <div className="flex w-full flex-row items-center justify-between opacity-50">
                  ...
                  <div className="flex items-center justify-end gap-1">
                    ...
                    {!isExpired && (
                      <JudgeAttestation
                        disabled={isPending || attestationData.userAttested}
                        ...
                        invalidAttestations={
                          attestationData.invalidAttestations
                        }
                        logId={hotdog.logId}
                        chainId={activeChain.id}
