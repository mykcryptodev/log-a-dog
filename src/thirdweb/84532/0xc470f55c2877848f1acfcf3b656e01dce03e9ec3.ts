...
/**
 * Represents the parameters for the "getAttestationCounts" function.
 */
export type GetAttestationCountsParams = {
  logIds: AbiParameterToPrimitiveType<{"internalType":"uint256[]","name":"logIds","type":"uint256[]"}>
};

/**
 * Calls the "getAttestationCounts" function on the contract.
 * @param options - The options for the getAttestationCounts function.
 * @returns The parsed result of the function call.
 * @example
 * ```
 * import { getAttestationCounts } from "TODO";
 *
 * const result = await getAttestationCounts({
 *  logIds: ...,
 * });
 *
 * ```
...
export async function getAttestationCounts(
  options: BaseTransactionOptions<GetAttestationCountsParams>
) {
  return readContract({
    contract: options.contract,
    method: [
...
/**
 * Represents the parameters for the "getAttestationStakes" function.
 */
export type GetAttestationStakesParams = {
  logIds: AbiParameterToPrimitiveType<{"internalType":"uint256[]","name":"logIds","type":"uint256[]"}>
};

/**
 * Calls the "getAttestationStakes" function on the contract.
 * @param options - The options for the getAttestationStakes function.
 * @returns The parsed result of the function call.
 * @example
 * ```
 * import { getAttestationStakes } from "TODO";
 *
 * const result = await getAttestationStakes({
 *  logIds: ...,
 * });
 *
 * ```
 */
export async function getAttestationStakes(
  options: BaseTransactionOptions<GetAttestationStakesParams>
) {
  return readContract({
    contract: options.contract,
    method: [
