...
import {
  DEFAULT_POIDH3_BOUNTY_URL,
  POIDH3_CAMPAIGN_DAYS,
  POIDH3_PRIZE_USD,
  POIDH3_WINNER_COUNT,
  getPoidh3BountyUrl,
} from "~/utils/poidh3";
...
const Poidh3Page: NextPage = () => {
  const farcaster = useContext(FarcasterContext);
  const isMiniApp = farcaster?.isMiniApp ?? false;
  const mounted = useMounted();
  const bountyUrl = mounted ? getPoidh3BountyUrl() : DEFAULT_POIDH3_BOUNTY_URL;
  ...
  return (
    <>
      <Head>
        <title>POIDH × Log a Dog — Labor Day Weekend Daily Winners</title>
        <meta
          name="description"
          content={`Win $${POIDH3_PRIZE_USD} ETH a day, September 4–7. One winner every day — log a hotdog, share it, claim your prize on POIDH.`}
        />
        ...
        <div className="flex w-full max-w-xl flex-col gap-5">
          ...
          <div className="text-center">
            ...
            <h1 className="font-display text-4xl font-bold tracking-tight sm:text-5xl">
              POIDH <span className="text-secondary">LABOR DAY</span>
            </h1>
            <p className="mt-2 text-sm opacity-70">
              Log a Dog meets &quot;Pics or it didn&apos;t happen&quot; — all Labor Day weekend long
            </p>
          </div>
          ...
          <div className="pop-card rounded-3xl bg-secondary p-5 text-secondary-content">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="font-display text-xs tracking-widest opacity-80">SEPTEMBER 4 – 7, 2026</p>
              ...
              <div className="shrink-0 text-center">
                <div className="font-display text-4xl font-bold">${POIDH3_PRIZE_USD}</div>
                <div className="font-display text-xs tracking-widest opacity-80">ETH / DAY</div>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
              {POIDH3_CAMPAIGN_DAYS.map((day) => (
                <div
                  key={day.date}
          ...
          <div className="pop-card rounded-3xl bg-base-100 p-5">
            ...
            <ul className="space-y-2">
              {[
                ...
                "Entry lands on a campaign day — September 4, 5, 6, or 7, 2026",
              ].map((req) => (
          ...
          {/* Link back to round two */}
          <Link
            href="/poidh-2"
            className="text-center text-xs font-semibold underline underline-offset-2 opacity-60 hover:opacity-100"
          >
            Missed round two? See the August campaign →
          </Link>

          {/* Fine print */}
          <p className="text-center text-xs opacity-40">
            Campaign runs Friday–Monday, September 4–7, 2026 (Labor Day weekend). One $
            {POIDH3_PRIZE_USD} ETH prize awarded per day. Winners selected by organizers
