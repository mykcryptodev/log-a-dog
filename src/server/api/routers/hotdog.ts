...
import {
  ...
  setCachedData,
  CACHE_DURATION,
  deleteCachedData /*, invalidateCache */,
...
import {
  getDogEvents,
  getDogEventsByEater,
  getDogEventLeaderboard,
} from "~/server/api/dog-events";
import { db } from "~/server/db";
import { getUserOpGasFees } from "thirdweb/wallets/smart";
import { abi } from "~/constants/abi/logadog";

const redactedImage =
  "https://ipfs.io/ipfs/QmXZ8SpvGwRgk3bQroyM9x9dQCvd87c23gwVjiZ5FMeXGs/Image%20(1).png";
...
async function getZoraCoinDetailsBatch(
  addresses: string[],
  chainId: number,
): Promise<Map<string, ZoraCoinDetails>> {
...
async function getAttestationPeriodsBatch(
  logIds: string[],
  chainId: number,
): Promise<
  Map<
  ...
  >
> {
  const attestationPeriodMap = new Map<
    string,
    {
      logId: string;
      startTime: string;
      ...
      const attestationPeriods = await Promise.all(
        chunk.map(async (logId) => {
          try {
            ...
            console.error(
              `Error fetching attestation period for log ${logId}:`,
              error,
            );
            return null;
          }
...
export const hotdogRouter = createTRPCRouter({
  getAll: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const contestStartTime = BigInt(
        new Date(CONTEST_START_TIME).getTime() / 1000,
      );
      const contestEndTime = BigInt(
        new Date(CONTEST_END_TIME).getTime() / 1000,
      );
      ...
      const logIds = dogEvents.map((event: any) => event.logId);
      ...
      const processedResponse: HotdogResponse = {
        ...
        validCounts: attestationCounts[0].map((count: any) => count.toString()),
        invalidCounts: attestationCounts[1].map((count: any) =>
          count.toString(),
        ),
        userHasAttested: dogEvents.map((event: any) =>
          userAttestations[0].includes(BigInt(event.logId)),
        ),
        userAttestations: dogEvents.map(
          (event: any) =>
            userAttestations[1][
              userAttestations[0].findIndex((id) => id === BigInt(event.logId))
            ] ?? false,
        ),
      };
      ...
      const processedHotdogs = processedResponse.logs.map((log) => {
        const zoraCoin =
          zoraCoinDetails.get(log.zoraCoin.toLowerCase()) ?? null;
  ...
  getAllForUser: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const filteredEvents = userDogEvents.filter(
        (event: any) =>
      ...
      const moderatedHotdogs = filteredEvents.map((event: any) => ({
        logId: event.logId,
        timestamp: event.timestamp.toString(),
        imageUri: redactedLogIdsStr.includes(event.logId)
          ? redactedImage
  ...
  getById: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const processedResponse: HotdogResponse = {
        logs: [
          {
            ...
            imageUri: redactedLogIdsStr.includes(dogEvent.logId)
              ? redactedImage
              : dogEvent.imageUri,
        ...
        userAttestations: [
          userAttestations[1][
            userAttestations[0].findIndex((id) => id === BigInt(dogEvent.logId))
          ] ?? false,
        ],
      };

      const zoraCoinAddressesArray = processedResponse.logs[0]?.zoraCoin
        ? [processedResponse.logs[0].zoraCoin]
        : [];
      ...
      const processedHotdog: ProcessedHotdog = {
        ...
        zoraCoin: processedResponse.logs[0]?.zoraCoin
          ? (zoraCoinDetails.get(
              processedResponse.logs[0].zoraCoin.toLowerCase(),
            ) ?? null)
          : null,
        metadata,
  ...
  getAiVerificationStatus: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const aiAttestation =
        userAttestations[1][
          userAttestations[0].findIndex((id) => id === BigInt(logId))
        ];
  ...
  checkForSafety: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        const safetyCheckResult: SafetyCheckResponse =
          (await response.json()) as SafetyCheckResponse;
        const safeSearchAnnotation =
          safetyCheckResult.responses[0]?.safeSearchAnnotation;
      ...
      try {
        ...
        const body = {
          ...
          txOverrides: {
            ...
            maxPriorityFeePerGas: (
              BigInt(maxPriorityFeePerGas) * 2n
            ).toString(), // 100 gwei (lower priority fee)
            timeoutSeconds: 10,
          },
        };
        console.log({ body });
  ...
  getDogEventByTransactionHash: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const { getDogEventByTransactionHash } = await import(
        "~/server/api/dog-events"
      );
